...
AppTitle "OneShot - Warehouse Breach"
...
Function CreateButton(x#,y#,z#, pitch#,yaw#,roll#=0)
	...
	ButtonTextureActive = LoadTexture_Strict("GFX\map\KeyPad_Active.png")
	ButtonTextureLocked = LoadTexture_Strict("GFX\map\KeyPad_Locked.png")
			...
			If d\open Then
				If d\openstate < 180 Then
					Select d\dir
						...
						Case DOOR_WINDOWED
							d\openstate = CurveValue(180.0, d\openstate, 40.0) + (FPSfactor * 0.01)
				...
				If d\openstate > 0 Then
					Select d\dir
						...
						Case DOOR_HCZ
							...
							If d\fastopen > 0 And d\openstate = 180 Then CameraShake = 3.0
...
Global Monitor%, MonitorTexture%, MonitorRec%
	...
	Else
		...
		If (Not MenuOpen)  Then
			If Sanity < 0 Then
				...
				If Sanity < (-200) Then 
					DarkAlpha = Max(Min((-Sanity - 200) / 700.0, 0.6), DarkAlpha)
					If KillTimer => 0 Then 
			...
			If EyeStuck > 0 Then 
				...
				If EyeStuck < 6000 Then darkA = Min(Max(DarkAlpha, (6000-EyeStuck)/5000.0),1.0)
			...
			If BlinkTimer < 0 Then
				If BlinkTimer > - 5 Then
					DarkAlpha = Max(darkA, Sin(Abs(BlinkTimer * 18.0)))
				ElseIf BlinkTimer > - 15
					DarkAlpha = 1.0
				Else
					DarkAlpha = Max(darkA, Abs(Sin(BlinkTimer * 18.0)))
				EndIf
			...
			If LightBlink > 0 Then DarkAlpha = Min(Max(darkA, LightBlink * Rnd(0.3, 0.8)), 1.0)
			
			If Using294 Then darkA=1.0
			
			If (Not WearingNightVision) Then DarkAlpha = Max((1.0-SecondaryLightOn)*0.9, darkA)
			...
			If KillTimer < 0 Then
				...
				EndIf
				DarkAlpha = Max(darkA, Min(Abs(KillTimer / 400.0), 1.0))
			EndIf
			...
			If FallTimer < 0 Then
				...
				DarkAlpha = Max(DarkAlpha, Min(Abs(FallTimer / 400.0), 1.0))				
			EndIf
			
			If SelectedItem <> Null Then
				If SelectedItem\itemtemplate\tempname = "navigator" Or SelectedItem\itemtemplate\tempname = "nav" Then DarkAlpha = Max(darkA, 0.5)
			End If
			If SelectedScreen <> Null Then DarkAlpha = Max(DarkAlpha, 0.5)
...
Function DrawGUI()
	...
	If ClosestItem <> Null Then
		temp = CreatePivot()
		PositionEntity temp, EntityX(Camera), EntityY(Camera), EntityZ(Camera)
		;PointEntity temp, ClosestItem\collider
		...
		AASetFont Font6
		Color(255,255,255)
		AAText(GraphicWidth / 2 + Sin(yawvalue) * (GraphicWidth / 3), GraphicHeight / 2 - Sin(pitchvalue) * (GraphicHeight / 3) - 58, ClosestItem\name, True)
			...
			Select OptionsMenu
				...
				Case 4 ;Advanced
					...
					If AATextEnable_Prev% <> AATextEnable
						...
						Font4% = AALoadFont("GFX\font\DS-DIGI\DS-Digital.ttf", Int(60 * (GraphicHeight / 1024.0)), 0,0,0)
						Font5% = AALoadFont("GFX\font\Journal\Journal.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0)
						Font6% = AALoadFont("GFX\font\cour\Courier New.ttf", Int(20), 0,0,0)
...
Function InitNewGame()
	...
	For r.Rooms = Each Rooms
		...
		If (Not r\RoomTemplate\DisableDecals) Then
			...
			If Rand(4) = 1 Then
				de.Decals = CreateDecal(DECAL_GLITCH_1, EntityX(r\obj)+Rnd(- 2,2), 0.003, EntityZ(r\obj)+Rnd(-2,2), 90, Rand(360), 0)
...
Select item\itemtemplate\name
	Case "Gas Mask", "Heavy Gas Mask"
		Select setting
			Case "rough", "coarse"
				d.Decals = CreateDecal(DECAL_GLITCH_1, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
	...
	Case "Cowbell"
		Select setting
			Case "rough","coarse"
				d.Decals = CreateDecal(DECAL_GLITCH_1, x, 8*RoomScale+0.010, z, 90, Rand(360), 0)
	...
	Case "Severed Hand", "Black Severed Hand"
		Select setting
			Case "rough", "coarse"
				d.Decals = CreateDecal(DECAL_BLOOD_2, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
				d\Size = 0.12 : ScaleSprite(d\obj, d\Size, d\Size)
			Case "1:1", "fine", "very fine"
				If (item\itemtemplate\name = "Severed Hand")
	...
	Case "SCP-714"
		Select setting
			...
			Case "fine"
				it2 = CreateItem("Blue Ring", "super714", x, y, z)
			Case "very fine"
